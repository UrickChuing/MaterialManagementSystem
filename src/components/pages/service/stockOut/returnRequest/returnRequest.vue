<template>
  <div @keydown.enter="initTableList" v-if="otherPage.PAGE == 'INDEX'">
    ...
    <div class="input-line">
      ...
      <div class="input-block">
        ...
        <el-select v-model="reason.status" :placeholder="$t('quanbu')" clearable>
          <el-option v-for="(item,statusIndex) in options.status" :key="item.value"
    ...
    <div class="public-table-header">
      <el-button @click="otherPage.PAGE = 'DETAILS';otherPage.status.details = STATUS.ADD" type="primary" size="mini">新增退货申请</el-button>
    ...
    <el-table :data="table.list" style="width: 100%" cell-class-name="public-table-cell">
      ...
      <el-table-column label="单号" prop="code" align="center" min-width="220">
        <template slot-scope="scope">
          <i-label :target="scope.row" :type="AuditType.value.DELIVERY_REQUEST" :info="false" key></i-label>
        </template>
      </el-table-column>
...
<script>
  import rDetails from './returnRequest_details.vue'
  ...
  export default {
    ...
    methods: {
      ...
      edit: function(row) {
        ...
        if (row.status === that.STATUS.UNCOMMIT || row.status == that.STATUS.NOPASS) {
