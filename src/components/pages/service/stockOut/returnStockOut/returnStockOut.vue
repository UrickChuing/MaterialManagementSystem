<template>
  <div @keydown.enter="initTableList" v-if="otherPage.PAGE == 'INDEX'">
    ...
    <div class="public-table-header">
      <el-button @click="otherPage.PAGE = 'DETAILS';otherPage.status.details = STATUS.ADD" type="primary" size="mini">新增退货出库</el-button>
    ...
    <el-table :data="table.list" style="width: 100%" cell-class-name="public-table-cell">
      ...
      <el-table-column label="单号" prop="code" align="center" min-width="200">
        <template slot-scope="scope">
          <i-label :target="scope.row" :type="AuditType.value.DELIVERY_VOUCHER" :info="false" key></i-label>
        </template>
      </el-table-column>
...
<script>
  import rDetails from './returnStockOut_details.vue'
      ...
      deleteFromList: function(row) {
        ...
        that.$confirm('您确定要删除吗？', '提示')
          .then(_ => {
            axios({
              method: 'post',
              url: '/delivery/voucher/return/delete',
              data: {
                targetId: row.id
              },
              df: true
