<template>
  <div @keydown.enter="initTableList" v-if="otherPage.PAGE == 'INDEX'">
    ...
    <div class="public-table-header">
      <el-button @click="otherPage.PAGE = 'DETAILS';otherPage.status.details = STATUS.ADD" type="primary" size="mini">新增换货出库</el-button>
    ...
    <el-table :data="table.list" style="width: 100%" cell-class-name="public-table-cell">
      ...
      <el-table-column label="单号" prop="code" align="center" min-width="210">
        <template slot-scope="scope">
          <i-label :target="scope.row" :type="AuditType.value.EXCHANGE_OUT" :info="false" key></i-label>
        </template>
      </el-table-column>
      <el-table-column label="所属入库单" prop="code" align="center" min-width="200">
        <template slot-scope="scope">
...
<script>
  import rDetails from './exchangeOut_details.vue';
  ...
  export default {
    ...
    methods: {
      ...
      submit: function(row) {
        ...
        axios({
          method: 'post',
          url: 'delivery/voucher/return/commit',
          data: {
            id: row.id,
