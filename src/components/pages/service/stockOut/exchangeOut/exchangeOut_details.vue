<template>
  <div>
    ...
    <div class="input-line">
      ...
      <div class="input-block">
        <span class="text-align-left">
          <el-button @click="openDeliveryRequestPop" type="warning" size="mini" style="padding: 5px 5px;" :disabled="operation != STATUS.ADD || operation == STATUS.READ">
            选择
          </el-button>
      ...
      <div class="input-block">
        <span>入库单</span>
        <el-input v-model="bean.warehouseVoucherCode" disabled></el-input>
      </div>
    ...
    <el-table :data="addItems" style="width: 100%" default-expand-all cell-class-name="public-table-cell" height="500">
      <el-table-column label="物品代码" prop="code" align="center" min-width="150">
        <template slot-scope="scope">
          <div class="font12">
            {{scope.row.materialPreCode}}<br>{{scope.row.materialCode}}
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="仓库" prop="warehouseName" align="center" min-width="180">
        <template slot-scope="scope">
          ...
          【<span class="blue">{{scope.row.warehouseTypeName == ''||scope.row.warehouseTypeName == undefined? '--':scope.row.warehouseTypeName}}</span>】
        </template>
      </el-table-column>
    ...
    <el-dialog :custom-class="['dialog-card','self-card']" :visible.sync="inAdd" :append-to-body="true">
      ...
      <delivery-request @deliveryReuqest="selectDeliveryRequest" :targetId="bean.deliveryRequestId" v-if="inAdd && inWhat == 'DR'"></delivery-request>
      <audit-process @set="setPorcess" :processId="bean.auditProcessId" :auditProcess="bean.auditProcess" :type="AuditType.value.EXCHANGE_OUT"
...
<script>
  import addItem from './exchangeOut_details_add.vue';
  import deliveryRequest from './exchangeOut_delivery_reqeust.vue';
    ...
    watch: {
      'bean.deliveryRequestId': {
        handler: function(newV, oldV) {
          if (oldV != '' && this.addItems.length != 0 && newV != oldV && this.operation != this.STATUS.READ) {
            this.addItems = [];
          }
        },
        deep: true
