<template>
  <div @keydown.enter="initTableList" v-if="otherPage.PAGE == 'INDEX'">
    ...
    <div class="public-table-header">
      <el-button @click="otherPage.PAGE = 'DETAILS';otherPage.status.details = STATUS.ADD" type="primary" size="mini">申请领取物料</el-button>
    ...
    <el-table :data="table.list" style="width: 100%" cell-class-name="public-table-cell"  >
      ...
      <el-table-column label="单号" prop="code" align="center" min-width="200">
        <template slot-scope="scope">
          <i-label :target="scope.row" :type="AuditType.value.RECEIVING_MATERIAL" :info="false" key></i-label>
        </template>
      </el-table-column>
...
<script>
  import rDetails from './getUsing_details.vue';
  ...
  export default {
    ...
    methods: {
      ...
      submit: function(row) {
        ...
        axios({
          ...
          setTimeout(function() {
            ...
            if (data.code == that.STATUS.SUCCESS) {
              that.$notify({
                title: '提交成功',
                message: '<div><span class="green">提交</span>物料申请成功!<br>单号:【' + row.code + '】</div>',
                type: 'success',
                duration: 5000,
        ...
        that.$confirm('是否确定已完成物料领取？', '提示')
          .then(_ => {

            axios({
              method: 'post',
              url: '/stock/out/getUsing/finish',
              data: {
                id: row.id
              }
            }).then((response) => {
              ...
              setTimeout(function() {
                ...
                if (data.code == that.STATUS.SUCCESS) {
                  that.$notify({
                    title: '完成成功',
                    message: '<div><span class="green">完成</span>物料领用成功!<br>单号:【' + row.code + '】</div>',
                    type: 'success',
                    duration: 5000,
                    dangerouslyUseHTMLString: true
                  });
