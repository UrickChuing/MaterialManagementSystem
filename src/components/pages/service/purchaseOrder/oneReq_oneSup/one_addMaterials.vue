    ...
    <div>
      ...
      <div class="public-scroll-y-hide" style="height: calc(100% - 41px);">
        ...
        <el-table :data="table.list" style="width: 100%;margin-top: 20px;border-top: 1px solid #EBEEF5;" max-height="490px"
          ...
          <el-table-column label="物品编码" prop="code" align="center" min-width="210">
          </el-table-column>
          <el-table-column label="部门" prop="organizationName" align="center" min-width="160">
          </el-table-column>
          ...
          <el-table-column label="邮轮库存" prop="applicantName" align="center" min-width="120">
            <template slot-scope="scope">
              {{ scope.row.availCruiseStock + '(' + scope.row.totalCruiseStock + ')'   }}
            </template>
          </el-table-column>
          <el-table-column label="总库存" prop="applicantName" align="center" min-width="120">
            <template slot-scope="scope">
              {{ scope.row.availStock + '(' + scope.row.totalStock + ')'   }}
            </template>
          </el-table-column>
  ...
  export default {
    data() {
      ...
      return {
        ...
        reason: {
          demandNumber: false,
          availCruiseStock: true,
          totalCruiseStock: true,
          availStock: true,
          totalStock: true,
          agreement: parent.bean.type == 0 ? false : true,
          purchase_request_material: true,
          task: true,

          supplierAgreementId: parent.bean.supplierAgreementId,

          cruiseStock_warehouseTypeId: 1,

          purchaseRequestId: parent.bean.purchaseRequestId,
    ...
    methods: {
      ...
      addItems: function(demo) {
        ...
        var price = demo.price;
        price = price == null || price == undefined || price == '' ?  0.00:price;
        var target = {
          materialId: demo.id,
          purchaseRequestId: demo.purchaseRequestId,
          purchaseRequestMaterialId: demo.purchaseRequestMaterialId,
          supplierAgreementId: demo.supplierAgreementId,
          supplierAgreementMaterialId: demo.supplierAgreementMaterialId,
          number: demo.number,
          remainNumber: demo.number,
          basisPrice: price,
          purchaseSpecification:demo.purchaseSpecification,
          materialCode: demo.code,
          materialName: demo.name,
          materialBarCode: demo.barCode,
          materialBrandName: demo.brandName,
          purchaseUnit: demo.purchaseUnit,
          materialCategoryName: demo.categoryName,
      ...
      initTableList: function() {
        ...
        axios({
          method: 'post',
          url: '/purchaseOrder/add/material/list',
          data: {
            reason: that.reason,
