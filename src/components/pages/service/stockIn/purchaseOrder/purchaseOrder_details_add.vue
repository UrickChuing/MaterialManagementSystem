    ...
    <div>
      ...
      <div class="public-scroll-y-hide" style="height: calc(100% - 41px);">
        ...
        <el-table :data="table.list" style="width: 100%;margin-top: 20px;border-top: 1px solid #EBEEF5;" max-height="490px"
          ...
          <el-table-column label="条码" prop="barCode" align="center" min-width="120">
          </el-table-column>
          ...
          <el-table-column label="采购订单剩余数量" prop="applicantName" align="center" min-width="140">
            <template slot-scope="scope">
              <span v-if="scope.row.remainNumber != 0">{{ scope.row.remainNumber }}</span>
              <span class="red" v-else>{{ scope.row.remainNumber }}</span>
            </template>
          </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      addItems: function(demo) {
        ...
        var expiration = demo.duration == 0 ? new Date('2099-12-31') : new Date(new Date().getTime() + demo.duration *
          1000 * 60 * 60 * 24);
        ...
        var target = {
          ...
          expiration: this.getCreateDateStr(expiration),
