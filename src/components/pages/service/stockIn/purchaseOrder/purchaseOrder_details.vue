  ...
  <div>
    ...
    <div class="input-line">
      <div class="input-block">
        <span>入库单号</span>
        <el-input value="自动生成" disabled v-if="operation == STATUS.ADD || operation == STATUS.READ"></el-input>
        <el-input v-model="bean.code" disabled v-else></el-input>
      </div>

      <div class="input-block">
        <span>创建人</span>
        <el-input :value="userInfo.name" disabled v-if="operation == STATUS.ADD || operation == STATUS.READ"></el-input>
      ...
      <div class="input-block">
        <span>供应商</span>
        <el-input v-model="bean.supplierName" placeholder="请选择采购订单" disabled></el-input>
      </div>

      <div class="input-block">
        <span>供应商代码</span>
        <el-input v-model="bean.supplierCode" placeholder="请选择采购订单" disabled></el-input>
      </div>

      <div class="input-block">
        <span>供应商协议</span>
        <el-input v-model="bean.supplierAgreementName" placeholder="请选择采购订单" disabled></el-input>
      </div>
      <!--
      <div class="input-block">
        <span>供应商协议</span>
        <el-select v-model="bean.supplierAgreementId" title="请选择供应商" placeholder="请先选择供应商协议"
      ...
      <div class="input-block">
        <span class="text-align-left">
          <el-button @click="openPurchaseOrder" type="warning" size="mini" style="padding: 5px 5px;" :disabled="bean.status != STATUS.UNCOMMIT || operation == STATUS.READ">
            选择
          </el-button>
          <span class="font10">采购订单</span>
        </span>
        <el-input v-model="bean.purchaseOrderCode" disabled title="采购订单" placeholder="采购订单" disabled>
        </el-input>
      </div>
    ...
    <el-table :data="addItems" style="width: 100%" default-expand-all cell-class-name="public-table-cell" height="500">
      ...
      <el-table-column align="center" min-width="420">
        ...
        <el-table-column label="含税单价" prop="basisPrice" align="center" min-width="151">
          <template slot-scope="scope">
        ...
        <el-table-column label="税率(%)" prop="basisPrice" align="center" min-width="150">
          <template slot-scope="scope">
            <el-input-number v-model="scope.row.taxRate" :min="0.00" style="width: 130px;" :precision="2" :step="1"
              controls-position="right" size="mini" :disabled="bean.warehouseTypeId=='' || bean.status != STATUS.UNCOMMIT || operation == STATUS.READ"></el-input-number>
          </template>
        </el-table-column>
      ...
      <el-table-column fixed="right" label="入库数量" prop="basisPrice" align="center" min-width="140">
        <template slot-scope="scope">
          <el-input-number v-model="scope.row.number" title="请选择邮轮" :min="1" style="width: 110px;" :step="1.0"
            controls-position="right" size="mini" :disabled="bean.status != STATUS.UNCOMMIT || operation == STATUS.READ"></el-input-number>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="过期时间" prop="basisPrice" align="center" min-width="190">
        <template slot-scope="scope">
      ...
      <el-table-column fixed="right" label="仓库" align="center" min-width="180">
        <template slot-scope="scope">
          <el-select v-model="scope.row.warehouseId" title="仓库" placeholder="请选择仓库" size="mini" :disabled=" bean.status != STATUS.UNCOMMIT || operation == STATUS.READ">
            <el-option v-for="item in options.warehouse" :key="item.id" :label="item.name" :value="item.id">
            </el-option>
          </el-select>
        </template>
    ...
    <el-dialog :custom-class="['dialog-card','self-card']" :visible.sync="inAdd" :append-to-body="true">
      ...
      <select-order slot @select="selectPurchaseOrder" :targetId="bean.purchaseOrderId" v-if="inAdd && inWhat == 'PO'"></select-order>
      <audit-process @set="setPorcess" :processId="bean.auditProcessId" :auditProcess="bean.auditProcess" :type="AuditType.value.STOCKIN_PURCHASE"
...
<script>
  import addItem from './purchaseOrder_details_add.vue';
  import selectOrder from './purchaseOrder_details_selectOrder.vue';
  ...
  export default {
    ...
    watch: {
      'bean.warehouseTypeId': {
        handler: function(newV) {
          ...
          if (newV != '') {
            this.initOptionsRoute(this.bean.warehouseTypeTaskId);
          } else {
            this.bean.warehouseTypeTaskId = '';
          }
        },
        deep: true
      },
      'bean.purchaseOrderId': {
        handler: function(newV, oldV) {
          if (oldV != '' && oldV != newV && this.operation != this.STATUS.READ) {
            this.addItems = []
          }
        },
        deep: true
    ...
    methods: {
      ...
      add: function(commit) {
        ...
        axios({
          ...
          data: {
            ...
            //采购订单
            purchaseOrderId: that.bean.purchaseOrderId,
            //说明
      ...
      selectPurchaseOrder: function(target) {
        this.bean.purchaseOrderId = target.id;
        this.bean.purchaseOrderCode = target.code;
        this.bean.supplierId = target.supplierId;
        this.bean.supplierName = target.supplierName;
        this.bean.supplierCode = target.supplierCode;
        this.bean.supplierAgreementId = target.supplierAgreementId;
        this.bean.supplierAgreementName = target.supplierAgreementName;
      },
