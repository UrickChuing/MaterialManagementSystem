<template>
  <div @keydown.enter="initTableList" v-if="otherPage.PAGE == 'INDEX'">
    ...
    <div class="input-line">
      ...
      <!--      <div class="input-block">
        <span>归还单号</span>
        <el-input v-model="reason.deliveryVoucherCode" clearable></el-input>
      </div> -->
    ...
    <div class="public-table-header">
      <el-button @click="otherPage.PAGE = 'DETAILS';otherPage.status = STATUS.ADD" type="primary" size="mini">归还入库</el-button>
    ...
    <el-table :data="table.list" style="width: 100%" cell-class-name="public-table-cell">
      <el-table-column label="单号" prop="code" align="center" min-width="210">
        <template slot-scope="scope">
          <i-label :target="scope.row" :type="AuditType.value.RECIVEING_MATERIAL_RETURN" :info="false" key></i-label>
        </template>
      </el-table-column>
...
<script>
  import sDetails from './getUsingBack_details.vue';
  ...
  export default {
    ...
    methods: {
      ...
      submit: function(row) {
        ...
        that.$confirm('是否确定已完成物料的归还？', '提示')
          .then(_ => {
            axios({
              method: 'post',
              url: '/stock/in/getBack/finish',
              data: {
                id: row.id
              }
            }).then((response) => {
              ...
              setTimeout(function() {
                ...
                if (data.code == that.STATUS.SUCCESS) {
                  that.$notify({
                    title: '完成成功',
                    message: '<div><span class="green">记入</span>领用归还成功!<br>单号:【' + row.code + '】</div>',
                    type: 'success',
                    duration: 5000,
                    dangerouslyUseHTMLString: true
                  });
                } else {
                  that.notic(false, "完成", data.msg);
                }
              }, 300)
