  ...
  <div>
    ...
    <div class="input-line">
      ...
      <div class="input-block">
        <span>
          <el-button @click="sysn('RATE')" type="warning" size="mini" style="padding: 5px 5px;margin-right: 10px;"
            :disabled="bean.warehouseTypeId=='' || bean.status != STATUS.UNCOMMIT || operation == STATUS.READ">同步</el-button>税率
        </span>
      ...
      <div class="input-block">
        <span>
          <el-button @click="sysn('WAREHOUSE')" type="warning" size="mini" style="padding: 5px 5px;margin-right: 10px;"
            :disabled="bean.warehouseTypeId=='' || bean.status != STATUS.UNCOMMIT || operation == STATUS.READ">同步</el-button>仓库
        </span>
        <el-select v-model="bean.warehouseId" title="仓库" placeholder="仓库" :disabled="bean.warehouseTypeId=='' || bean.status != STATUS.UNCOMMIT || operation == STATUS.READ">
      ...
      <div class="input-block">
        <span class="text-align-left">
          <el-button @click="openDeliveryRequestPop" type="warning" size="mini" style="padding: 5px 5px;" :disabled="bean.status != STATUS.UNCOMMIT || operation == STATUS.READ">
            选择
          </el-button>
          <span class="font10">换货申请</span>
        </span>
        <el-input v-model="bean.deliveryRequestCode" disabled title="换货申请" placeholder="换货申请" disabled>
        </el-input>
      </div>
      ...
      <div class="input-block">
        ...
        <el-input v-model="bean.description" style="width: 535px" clearable :disabled="bean.warehouseTypeId=='' || bean.status != STATUS.UNCOMMIT || operation == STATUS.READ"></el-input>
      </div>
    ...
    <el-table :data="addItems" style="width: 100%" default-expand-all cell-class-name="public-table-cell" height="500">
      ...
      <el-table-column fixed="right" label="过期时间" prop="basisPrice" align="center" min-width="180">
        <template slot-scope="scope">
          <el-date-picker v-model="scope.row.expiration" value-format="yyyy-MM-dd" type="date" placeholder="选择日期" style="width: 150px;"
            size="mini" :disabled="bean.warehouseTypeId=='' || bean.status != STATUS.UNCOMMIT || operation == STATUS.READ">
          </el-date-picker>
        </template>
      </el-table-column>
      ...
      <el-table-column fixed="right" label="仓库" align="center" min-width="180">
        <template slot-scope="scope">
          <el-select v-model="scope.row.warehouseId" title="仓库" placeholder="仓库" size="mini" :disabled="bean.warehouseTypeId=='' || bean.status != STATUS.UNCOMMIT || operation == STATUS.READ">
            <el-option v-for="item in options.warehouse" :key="item.id" :label="item.name" :value="item.id">
            </el-option>
          </el-select>
        </template>
    ...
    <el-dialog :custom-class="['dialog-card','self-card']" :visible.sync="inAdd" :append-to-body="true">
      ...
      <delivery-request slot @deliveryReuqest="selectDeliveryRequest" :targetId="bean.deliveryRequestId" v-if="inAdd && inWhat == 'DR'"></delivery-request>
      <audit-process @set="setPorcess" :processId="bean.auditProcessId" :auditProcess="bean.auditProcess" :type="AuditType.value.EXCHANGE_IN"
...
<script>
  import addItem from './exchangeIn_details_add.vue';
  import deliveryRequest from './exchangeIn_delivery_reqeust.vue';
  ...
  export default {
    ...
    watch: {
      'bean.warehouseTypeId': {
        handler: function(newV) {
          if (newV != '' && newV != undefined && this.operation != this.STATUS.READ) {
    ...
    methods: {
      ...
      initOptions: function(callBack) {
        ...
        axios({
          ...
          if (callBack != undefined && callBack != null) {
            ...
            that.bean.warehouseTypeId = cruise[0] != undefined ? cruise[0].id : '';
          }
      ...
      initOptionsAgreement: function() {
        ...
        axios({
          method: 'post',
          url: '/exchange/in/options/agreement',
          data: {
      ...
      selectDeliveryRequest: function(target) {
        ...
        that.bean.deliveryRequestId = target.id;
        that.bean.deliveryRequestCode = target.code;
      ...
      openAddPop: function() {
        ...
        if (that.bean.deliveryRequestId == '') {
          ...
          that.$notify({
            title: '警告',
            type: 'warning',
            message: '请选择<span class="blue">换货申请</span>',
            duration: 2500,
            dangerouslyUseHTMLString: true
          })
      ...
      openDeliveryRequestPop: function() {
        var that = this;
