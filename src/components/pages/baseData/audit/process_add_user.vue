      ...
      <el-table-column label="操作" prop="applicationNo" align="center" width="150">
        ...
        <template slot-scope="scope">
          <div class="flexCenter margin-right30" v-if="scope.row.number == 0">
            <el-button @click="addUser(scope.row)" size="mini" icon="el-icon-plus" type="primary" circle></el-button>
          </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      mustUser: function(must,row){
        var that = this;
        var axios = that.axios;

        axios({
          method: 'post',
          url: '/audit/process/bean/step/user/must',
          data: {
            id: row.auditProcssStepUserId,
            essential: must
          },
          df: false,
          loading:true
